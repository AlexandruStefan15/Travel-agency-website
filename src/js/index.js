...
const products = data.products.map((product) => ({
	...product,
	date_departure: setCurrentYear(product.date_departure),
	date_arrival: setCurrentYear(product.date_arrival),
}));
				...
				<div class="card-footer">
					<a href="#">
						...
						<svg
							...
							viewBox="0 0 24 24"
						>
							<path d="M2 24h20v-14h-20v14zm4-11h12v1h-12v-1zm0 3h12v1h-12v-1zm0 3h12v1h-12v-1zm16-17v6h-20v-6h3v1c0 1.103.897 2 2 2s2-.897 2-2v-1h6v1c0 1.103.897 2 2 2s2-.897 2-2v-1h3zm-14 1c0 .552-.447 1-1 1s-1-.448-1-1v-2c0-.552.447-1 1-1s1 .448 1 1v2zm10 0c0 .552-.447 1-1 1s-1-.448-1-1v-2c0-.552.447-1 1-1s1 .448 1 1v2z"/>
						</svg>
					</a>
...
function showProducts() {
	let button = document.querySelector("main section.products .container .see-more button");

	let hiddenProducts = document.querySelectorAll(".products .container ul > li.hide");

	if (hiddenProducts.length <= 9) {
...
function noResults() {
	...
	let message = document.querySelector("main section.products .container p.no-results-message");
...
function filter() {
	let seeMoreButton = document.querySelector("main section.products .container .see-more button");
	...
	let minPrice = document.querySelector("main .info-banner form input.min_value.numberVal");

	let maxPrice = document.querySelector("main .info-banner form input.max_value.numberVal");
	...
	if (form) {
		form.addEventListener("submit", (e) => {
			...
			for (let i = 0; i < products.length; i++) {
				...
				let condition2 =
					Date.parse(products[i].date_departure.split(".").reverse().join("-")) >= Date.parse(departureAfter.value);

				let condition3 =
					Date.parse(products[i].date_departure.split(".").reverse().join("-")) <= Date.parse(departureBefore.value);
				...
				if (departureAfter.value == 0) {
					condition2 = true;
				}

				if (departureBefore.value == 0) {
					condition3 = true;
				}
				...
				if (
					...
					el.innerHTML = `
						...
						<div class="card-footer">
							...
							<span>
								Plecare din ${products[i].airport}
							</span>
						</div>
					`;
			...
			document.querySelector("main section.products .container h1").innerText = "Rezultatele cautarii:";
